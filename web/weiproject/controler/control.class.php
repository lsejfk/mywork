...
class Control{
    ...
    public function submit_rec_data($user_id=null){
        ...
        */
        ...
        if($page == "add_coupon.php"){
            ...
            if(!$this->bandcar($user_id)){
                if($this->modulor->submit_data($data, "car")){
                    info_to_user("绑定成功！","success","确定","/weipro/coupon.php");
                }else{
                    info_to_user("绑定失败！","info","重试");
                }
            }else{
                info_to_user("您已经绑定了车牌号！");
            }
        }
    }
    ...
    //get coupon title data for the option of reward couponget_car_info
    ...
    public function bandcar($user_id){
        return $this->modulor->modbandcar($user_id);
    }

    //get the user's car information
    public function get_car_info($user_id){
        $data = $this->modulor->modget_car_info($user_id);
        
        if(empty($data)){
            info_to_user("您还未绑定车牌号，请先绑定车牌信息！","info","去绑定","/weipro/manage/add_car.php",true);
        }
        $res = array();
        $res['car_id'] = $data['car_id'];
        $res['car_type'] = $data['car_type'];
        $res['car_color'] = $data['car_color'];
        return $res;
    }
}
